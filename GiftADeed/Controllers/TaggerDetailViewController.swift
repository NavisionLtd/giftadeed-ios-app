...
extension Notification.Name {
    static let postNotifi = Notification.Name("removecustomeinfo")
}
class TaggerDetailViewController: UIViewController, CLLocationManagerDelegate, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate{
   ...
   // @IBOutlet  var outletDiscription: UILabel!
    ...
    override func viewDidLoad() {
        ...
        outletCommentTableView.layer.borderColor = UIColor.black.cgColor
        ...
        self.storyOfNeedView.layer.borderColor = UIColor.black.cgColor
//setText()
  ...
  func findCurrentLocation(){
            ...
            if let jsonObj = try? JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? NSDictionary {
                ...
                DispatchQueue.main.async {
                    ...
                    if let deed_details = jsonObj!.value(forKey: "deed_details") as? NSArray {
                        ...
                        DispatchQueue.main.async{
                            self.group = ((deed_details[0] as AnyObject).value(forKey: "group_name") as? String)!
                            self.cat_type = ((deed_details[0] as AnyObject).value(forKey: "cat_type") as? String)!
                            self.groupid = ((deed_details[0] as AnyObject).value(forKey: "group_id") as? String)!
                            ...
                            self.iconURL = String(format: "%@%@", Constant.BASE_URL , (deed_details[0] as AnyObject).value(forKey:"imgUrl") as! String)
                            self.charactorURL = String(format: "%@%@", Constant.BASE_URL ,(deed_details[0] as AnyObject).value(forKey:"characterPath") as? String ?? "char")
                            if((deed_details[0] as AnyObject).value(forKey:"imgUrl") as! String == ""){
                                self.outletProfilePic.contentMode = .scaleAspectFit
                            }else{
                                self.outletProfilePic.contentMode = .scaleToFill
                            }
                            
                            self.outletProfilePic.sd_setImage(with: URL(string: self.iconURL), placeholderImage: UIImage(named: "default"))
                            self.outletCharacterImg.sd_setImage(with: URL(string: self.charactorURL), placeholderImage: UIImage(named: "default"))
                          ...
                          print(self.is_endorse)
                            ...
                            self.outletTagName.text = ("\(self.need)")
                            print(self.subtype)
                          ...
                          //  self.preferenceTitleText.addBottomBorder(UIColor.black, height: 1.0)
                            self.subTyeText.text = self.subtype
                            ...
                            if(self.cat_type == "C"){
                                ...
                                if self.need.isEqual("Water") || self.need.isEqual("Food"){
                                    ...
                                    if containerStatus.isEqual("1") {
                                        ...
                                        // self.containerLabel.text = "Container : Available"
                                        //  self.outletContainerAvailable.text = "Yes"
                                    }
                                    else{
                                        self.containerLabel.isHidden = false
                                        //   self.outletLabelSpacing.constant = 21
                                        self.containerLabel.text = "Container : Not available"
                           ...
                           self.outletDistance.text = String(format: "Get Directions %.04f km(s) away", distanceInKM)
                            ...
                            if privacyStaus.isEqual("Public"){
                                ...
                                self.outletTagedBy.text = ("Deed tagged by : \(fullName)")
                            }
                            else{
                                
                                self.outletTagedBy.text = ("Deed tagged by : Anonymous ")
                            }
                              self.taggedByGroup.text = ("Tagged by group : \(self.group)")
                            ...
                            if(self.discriptionStr.count>0){
                                 ...
                                 self.outletDiscription.text = "Not avilable"
                            }
                          self.outletViews.text =  (deed_details[0] as AnyObject).value(forKey: "views") as? String
                           self.outletLikes.text =  (deed_details[0] as AnyObject).value(forKey: "endorse") as? String
                            ...
                            if(self.outletLikes.text == "0"){
                                ...
                                self.endorseDate.text = ("Last endorsed on : \(((deed_details[0] as AnyObject) .value(forKey:"last_endorse_time") as! String))")
                            }
    ...
    func updateUI() {
        ...
        if  self.commentsArr.count != 0{
         ...
         //  self.reviewHeightConstraint.constant = 188
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?!.outletMessageLabel.text = (item as AnyObject).value(forKey:"comment") as? String
        
        let privacyType = (item as AnyObject).value(forKey:"privacy") as! String
        if privacyType.isEqual("Public"){
            
            cell?!.outletNameLabel.text = (item as AnyObject).value(forKey:"fName") as? String
        }
        else{
            
            cell?!.outletNameLabel.text = "Anonymous"
        }
...
print("\(userId)\(self.ownerId)")
        if self.ownerId.isEqual(userId){
            ...
            self.view.hideAllToasts()
            self.navigationController?.view.makeToast("You cannot endorse your own tag.")
            return
        }
        ...
        if self.is_endorse == 1{
            ...
            self.view.hideAllToasts()
            self.navigationController?.view.makeToast("You have already endorse.")
            return
        }
       ...
       // self.outletLoadingView.isHidden = false
        ...
        if Int(distanceInFeet)<=dist ?? 100{
            ...
            self.view.hideAllToasts()
            self.navigationController?.view.makeToast("You need to be within \(self.endorseDist) feet area of the needy person")
            return
        }
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                       ...
                       // self.animationView.isHidden=true
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            ...
                            self.view.hideAllToasts()
                            self.navigationController?.view.makeToast("Thank you for endorsing.")
                            self.viewDidLoad()
                        }
    ...
    @IBAction func reportUserAction(_ sender: Any) {
      ...
      //  self.outletLoadingView.isHidden = false
        ...
        if userId.isEqual(self.taggerID){
           ...
           // self.outletLoadingView.isHidden = true
            self.view.hideAllToasts()
            self.navigationController?.view.makeToast("You cannot report yourself")
            return;
        }
        else if self.reported ==  1{
            
            //self.outletLoadingView.isHidden = true
            self.navigationController?.view.makeToast("You already reported")
            return;
        }
            ...
            if let jsonObj = try? JSONSerialization.jsonObject(with: data!, options: .allowFragments) as? NSDictionary {
                ...
                if status==1{
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.view.hideAllToasts()
                        self.navigationController?.view.makeToast("User reported successfully")
                    }
                }
                else{
       ...
       // self.outletLoadingView.isHidden = false
        ...
        if userId.isEqual(self.taggerID){
            ...
            DispatchQueue.main.async {
                ...
                //self.outletLoadingView.isHidden = true
                self.view.hideAllToasts()
                self.navigationController?.view.makeToast("You cannot report your own deed")
            }
            return;
        }
