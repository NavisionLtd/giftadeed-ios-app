...
func writePlist(namePlist: String, key: String, data: AnyObject){
    ...
    if let dict = NSMutableDictionary(contentsOfFile: path){
        ...
        if dict.write(toFile: path, atomically: true){
            print("plist_write")
            // Post notification
            NotificationCenter.default.post(name: notificationsuccessName, object: nil)
        }else{
            // Post notification
            NotificationCenter.default.post(name: notificationfailName, object: nil)
    ...
    func emptyPlist(namePlist: String, key: String, data: AnyObject){
        let paths = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true) as NSArray
        let documentsDirectory = paths.object(at: 0) as! NSString
        let path = documentsDirectory.appendingPathComponent(namePlist+".plist")
