...
/*
 What: Location based notification issue?
 How: We have to update user location every time even in kill state to solve this issue i am using corelocation  method and methods are written in obj c with LocationManager
 Where: 
*/
...
- (void)addLocationToPList:(BOOL)fromResume {
    ...
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"loginFlag"] isEqualToString:@"TRUE"]){
        
        [self updateLocationWithLatitude:[NSString stringWithFormat:@"%f",self.myLocation.latitude] andLongitude:[NSString stringWithFormat:@"%f",self.myLocation.longitude]];
    }
...
-(void)updateLocationWithLatitude:(NSString*)latitude andLongitude:(NSString*)longitude{
    ...
    NSURL *url1 = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",[defaults valueForKey:@"BASE_URL"],[defaults valueForKey:@"UPDATE_LOCATION"]]];
    ...
    NSString * radiusVal = [[NSUserDefaults standardUserDefaults] valueForKey:@"DEED_RADIUS"];
    NSString * deviceToken = [[NSUserDefaults standardUserDefaults] valueForKey:@"FCMTOEKN"];
    
    NSString *postParams =[NSString stringWithFormat:@"user_id=%@&device_id=%@&lat=%@&lng=%@&radius=%@",userID,deviceToken,latitude,longitude,radiusVal];
